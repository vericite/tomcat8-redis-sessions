 ...
 */
public class RedisSessionManagerBase implements Manager {

  private static final Log LOG = LogFactory.getLog(RedisSessionManagerBase.class);
  ...
  public void setRedis(Redis redis) {
    Objects.requireNonNull(redis, "redis pool for use with this manager");
    this.redis = redis;
  }
  ...
  protected Long getSessionAliveTime(String sessionId) {
    ...
    try {
      List<String> results = redis.withRedis(jedis -> {
        return jedis.hmget(Convention.sessionIdToMetadataKey(sessionId), Convention.CREATION_TIME_HKEY, Convention.EXPIRED_TIME_HKEY);
      });
  ...
  protected int calculateSessionAverageAliveTime() {
    try {
      Double average = createExpiredSessionIdStream()
    		  .mapToLong(this::getSessionAliveTime)
    		  .average()
    		  .orElse(0L);
  ...
  protected Date getSessionExpirationDate(String sessionId) {
    ...
    try {
      List<String> results = redis.withRedis(jedis -> {
        return jedis.hmget(metadataKey, Convention.LAST_ACCESS_TIME_HKEY);
      });
      ...
      return new Date(lastAccessTime.getTime() + TimeUnit.SECONDS.toMillis(maxInactiveInterval));
    } catch (Exception e) {
